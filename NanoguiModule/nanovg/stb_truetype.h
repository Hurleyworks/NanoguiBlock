   ...
   if (format == 0)   // apple byte encoding
   {
      ...
      if (format == 6)
      {
         ...
         if (format == 2)
         {
            ...
            if (format == 4)   // standard mapping for windows fonts: binary search collection of ranges
            {
      ...
      if (numberOfContours == -1)
      {
         ...
         while (more)
         {
            ...
            if (flags & (1 << 3)) // WE_HAVE_A_SCALE
            {
               ...
               if (flags & (1 << 6)) // WE_HAVE_AN_X_AND_YSCALE
               {
                  ...
                  if (flags & (1 << 7)) // WE_HAVE_A_TWO_BY_TWO
                  {
                     mtx[0] = ttSHORT (comp) / 16384.0f;
                     comp += 2;
            ...
            if (matchlen >= 0)
            {
               ...
               if (i + 1 < count && ttUSHORT (fc + loc + 12 + 6) == next_id && ttUSHORT (fc + loc + 12) == platform && ttUSHORT (fc + loc + 12 + 2) == encoding && ttUSHORT (fc + loc + 12 + 4) == language)
               {
                  ...
                  if (slen == 0)
                  {
                     ...
                     if (matchlen < nlen && name[matchlen] == ' ')
                     {
                        ++matchlen;
