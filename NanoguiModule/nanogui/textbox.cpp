...
void TxtBox::draw (NVGcontext * ctx)
{
   ...
   NVGpaint bg = nvgBoxGradient (ctx,
                                 ...
                                 3, 4, Colour (255, 32), Colour (32, 32));
   NVGpaint fg1 = nvgBoxGradient (ctx,
                                  mPos.x + 1, mPos.y + 1 + 1.0f, mSize.x - 2, mSize.y - 2,
                                  3, 4, Colour (150, 32), Colour (32, 32));
   ...
   nvgRoundedRect (ctx, mPos.x + 0.5f, mPos.y + 0.5f, mSize.x - 1,
                   mSize.y - 1, 2.5f);
   nvgStrokeColor (ctx, Colour (0, 48));
   nvgStroke (ctx);
   nvgFontSize (ctx, fontSize());
   nvgFontFace (ctx, "sans");
   ivec2 drawPos (mPos.x, mPos.y + mSize.y * 0.5f + 1);
   ...
   if (mUnitsImage > 0)
   {
      ...
      if (!mUnits.empty())
      {
         ...
         nvgFillColor (ctx, Colour (255, mEnabled ? 64 : 32));
