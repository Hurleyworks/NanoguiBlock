...
ivec2 BoxLayout::preferredSize (NVGcontext * ctx, const Widget * widget) const
{
   ivec2 size = ivec2 (2 * mMargin);
   int yOffset = 0;
   ...
   for (auto w : widget->children())
   {
      ...
      if (first)
         ...
         size[axis1] += mSpacing;
      ivec2 ps = w->preferredSize (ctx), fs = w->fixedSize();
      ivec2 targetSize (
         fs[0] ? fs[0] : ps[0],
         fs[1] ? fs[1] : ps[1]
      );
...
ivec2 GroupLayout::preferredSize (NVGcontext * ctx, const Widget * widget) const
{
   ...
   for (auto c : widget->children())
   {
      ...
      first = false;
      ivec2 ps = c->preferredSize (ctx), fs = c->fixedSize();
      ivec2 targetSize (
         fs[0] ? fs[0] : ps[0],
         fs[1] ? fs[1] : ps[1]
      );
...
void GroupLayout::performLayout (NVGcontext * ctx, Widget * widget) const
{
   ...
   for (auto c : widget->children())
   {
      ...
      first = false;
      bool indentCur = indent && label == nullptr;
      ivec2 ps = ivec2 (availableWidth - (indentCur ? mGroupIndent : 0),
                              c->preferredSize (ctx).y);
      ivec2 fs = c->fixedSize();
      ivec2 targetSize (
         fs[0] ? fs[0] : ps[0],
         fs[1] ? fs[1] : ps[1]
      );
      c->setPosition (ivec2 (mMargin + (indentCur ? mGroupIndent : 0), height));
...
ivec2 GridLayout::preferredSize (NVGcontext * ctx,
...
void GridLayout::computeLayout (NVGcontext * ctx, const Widget * widget, std::vector<int> * grid) const
{
   ...
   for (int i2 = 0; i2 < dim[axis2]; i2++)
   {
      for (int i1 = 0; i1 < dim[axis1]; i1++)
      {
         ...
         ivec2 ps = w->preferredSize (ctx);
         ivec2 fs = w->fixedSize();
         ivec2 targetSize (
            fs[0] ? fs[0] : ps[0],
            fs[1] ? fs[1] : ps[1]
         );
...
void GridLayout::performLayout (NVGcontext * ctx, Widget * widget) const
{
   ...
   int axis1 = (int) mOrientation, axis2 = (axis1 + 1) % 2;
   ivec2 start = ivec2 (mMargin) + extra;
...
ivec2 AdvancedGridLayout::preferredSize (NVGcontext * ctx, const Widget * widget) const
{
   /* Compute minimum row / column sizes */
   std::vector<int> grid[2];
   computeLayout (ctx, widget, grid);
...
void AdvancedGridLayout::performLayout (NVGcontext * ctx, Widget * widget) const
{
   ...
   for (int axis = 0; axis < 2; ++axis)
   {
      ...
      for (Widget * w : widget->children())
      {
         ...
         ivec2 pos = w->position(), size = w->size();
         pos[axis] = itemPos;
