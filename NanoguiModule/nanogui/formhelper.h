...
/**
   ...
   h->addWindow(Eigen::ivec2(10,10),"Menu");
...
class FormHelper
{
   public:
      ...
      Window * addWindow (const ivec2 & pos,
                   ...
                   const std::function<Type()> & getter, bool editable = true)
      {
         ...
         ivec2 fs = widget->fixedSize();
         widget->setFixedSize (ivec2 (fs.x != 0 ? fs.x : mFixedSize.x,
      ...
      void setFixedSize (const ivec2 & fw)
      {
         mFixedSize = fw;
      }
      ivec2 fixedSize()
...
template <typename T> class FormWidget<T, typename std::is_integral<T>::type> : public IntBox<T>
{
   public:
      ...
      {
         this->setAlignment (TxtBox::Alignment::Right);
      }
};
...
template <> class FormWidget<std::string, std::true_type> : public TxtBox
{
   public:
      FormWidget (Widget * p) : TxtBox (p)
      {
         setAlignment (TxtBox::Alignment::Left);
      }
      void setCallback (const std::function<void (const std::string &)> & cb)
      {
         TxtBox::setCallback ([cb] (const std::string & str)
         {
            cb (str);
            return true;
         });
...
template <> class FormWidget<Colour, std::true_type> : public ColorPicker
{
   public:
      FormWidget (Widget * p) : ColorPicker (p) { }
      void setValue (const Colour & c)
      {
         setColor (c);
      }
      void setEditable (bool e)
